...
function ExportCard({ card, holoProps, onReady }) {
  ...
  const exportSide = card.back ? 'back' : 'front';
  return (
    <div className="export-card" style={{ width: 360, height: 504, background: 'transparent' }}>
      <HoloCard
        card={card.back ? { rank: 'A', suit: 'spades' } : card}
        size="lg"
        interactive={false}
        enableDrag={false}
        flatMode={true}
